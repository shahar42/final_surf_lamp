...
#define BRIGHTNESS 30  //low for weak USB sources 
...
void showWaitingForDataIndicator(CRGB color);
...
void showWaitingForDataIndicator(CRGB color = CRGB::Blue) {
    ...
    float brightnessFactor = 0.3 + 0.7 * sin(phase);
    int adjustedBrightness = min(MAX_BRIGHTNESS, (int)(MAX_BRIGHTNESS * brightnessFactor));

    FastLED.clear();
    CHSV hsvColor = rgb2hsv_approximate(color);
    leds_center[1] = CHSV(hsvColor.hue, hsvColor.sat, adjustedBrightness);
    FastLED.show();
}
...
void setup() {
    ...
    //wifiManager.resetSettings();  // ENABLED - testing WiFi config makes have to configure each new start
...
void loop() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        if (lastSurfData.dataReceived && (millis() - lastSurfData.lastUpdate < 1800000)) { // 30 minutes
            // Check if all surf data is zero (no real conditions available)
            bool allZeros = (lastSurfData.waveHeight == 0.0 && lastSurfData.wavePeriod == 0.0 && lastSurfData.windSpeed == 0.0);

            if (allZeros) {
                // Data received but all zeros - show yellow waiting indicator
                showWaitingForDataIndicator(CRGB::Yellow);
                blinkStatusLED(CRGB::Yellow);
            } else {
                // Fresh real data - update blinking LEDs if any thresholds are exceeded
                updateBlinkingAnimation();
                blinkGreenLED(); // Fresh data
            }
        } else {
            // No data received yet - show blue waiting indicator
            showWaitingForDataIndicator(CRGB::Blue);
