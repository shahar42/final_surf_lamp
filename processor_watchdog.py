...
- Checks database heartbeat timestamp every T_INTERVAL
- Increments missed_count if stale
- Triggers Render restart if missed_count > MAX_MISSED_PINGS
- Stops after MAX_REVIVES to prevent infinite loops
"""
...
# Watchdog Configuration (matching C implementation constants)
...
RENDER_API_KEY = os.environ.get('RENDER_API_KEY', 'rnd_j6WyOXSMG0bGFbqyYKMBaxUgzF4s')
RENDER_SERVICE_ID = os.environ.get('RENDER_PROCESSOR_SERVICE_ID')  # Set this in .env
...
def check_heartbeat_staleness(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            # Processor should write heartbeat every 60s, we check every 120s
            # Stale if no update in last 180s (3 minutes = 3 heartbeat cycles)
            staleness_threshold = 180  # 3 minutes
...
def increment_missed_count(engine):
    """Increment missed_count in database (equivalent to g_wd_counter++)"""
    query = text("""
...
def increment_revive_count(engine):
    """Increment revive_count after restart (equivalent to data->revive_counter++)"""
    query = text("""
        UPDATE processor_heartbeat
...
def restart_processor_service():
    """
    Restart the processor via Render API (equivalent to ReviveClient)
...
def watchdog_check_cycle(engine):
    """
    Single watchdog check cycle (equivalent to CheckClientAlive task)

    Logic mirrors wd_launch.c:CheckClientAlive():
    1. Check if heartbeat is stale
    2. If stale, increment missed_count
    3. If missed_count > MAX_MISSED_PINGS, attempt revival
    4. If revive_count >= MAX_REVIVES, stop monitoring
    """
    ...
    if status['is_stale']:
        ...
        # Increment missed count (equivalent to ++g_wd_counter)
        ...
        # Check if exceeded max missed pings
        if missed_count > MAX_MISSED_PINGS:
            # Check if we've exceeded max revives
            ...
            # Attempt revival (equivalent to ReviveClient)
            ...
            if restart_processor_service():
                ...
                # Reset missed count for next cycle
...
def main():
    ...
    if not RENDER_SERVICE_ID:
        logger.error("❌ RENDER_PROCESSOR_SERVICE_ID not set in environment")
        logger.error("   Set this to enable automatic restarts")
        sys.exit(1)
    ...
    try:
        ...
        while True:
            ...
            # Sleep for T_INTERVAL before next check
