...
void loadCredentials() {
    ...
    Serial.printf("📝 Loaded credentials - SSID: %sn", ssid);
}
...
bool connectToWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.printf("📍 IP Address: %sn", WiFi.localIP().toString().c_str());
        Serial.printf("📶 SSID: %sn", WiFi.SSID().c_str());
        Serial.printf("💪 Signal Strength: %d dBmn", WiFi.RSSI());
...
void startConfigMode() {
    ...
    Serial.printf("📍 AP IP: %sn", WiFi.softAPIP().toString().c_str());
    ...
    server.on("/save", HTTP_POST, []() {
        if (server.hasArg("ssid") && server.hasArg("password")) {
            ...
            Serial.printf("🔄 Connecting to: %sn", newSSID.c_str());
            ...
            if (WiFi.status() == WL_CONNECTED) {
                Serial.println("n✅ Connected to new WiFi!");
                Serial.printf("📍 IP Address: %sn", WiFi.localIP().toString().c_str());
                configure_wifi = false;
                setupHTTPEndpoints();
            } else {
                Serial.println("n❌ Failed to connect to new WiFi");
...
/**
 * @brief Processes the JSON data received from the server.
 * 
 * This function was corrected to use the full JSON keys sent by the server
 * (e.g., "wave_height_cm") instead of incorrect short keys (e.g., "h").
 * It now correctly parses the payload documented in arduino_payload_documentation.md.
 */
bool processSurfData(const String &jsonData) {
    ...
    if (error) {
        Serial.printf("❌ JSON parsing failed: %sn", error.c_str());
        return false;
    }
    ...
    Serial.printf("   Wave Height: %d cmn", wave_height_cm);
    Serial.printf("   Wave Period: %.1f sn", wave_period_s);
    Serial.printf("   Wind Speed: %d m/sn", wind_speed_mps);
    Serial.printf("   Wind Direction: %d°n", wind_direction_deg);
    Serial.printf("   Wave Threshold: %d cmn", wave_threshold_cm);
...
/**
 * @brief Updates the LED display based on surf conditions.
 * 
 * This function was corrected to accept integer values for wave height and threshold
 * in centimeters (cm), matching the data sent by the server. This avoids
 * unnecessary floating-point math on the microcontroller.
 */
void updateSurfDisplay(int waveHeight_cm, float wavePeriod, int windSpeed, int windDirection, int waveThreshold_cm = 100) {
    ...
    Serial.printf("🎨 LEDs Updated - Wind: %d, Wave: %d, Period: %d, Direction: %d° [Threshold: %dcm]n", 
...
void setup() {
    ...
    Serial.println("🌊 ========================================");
    Serial.printf("🔧 Arduino ID: %dn", ARDUINO_ID);
